...
(function () {
  ...
  var CFG = {
    ...
    // Defaults to /members rather than the homepage — landing back on the
    // homepage after a successful login reads as "did nothing." Still fully
    // overridable per site with data-after-login.
    afterLogin: (script && script.dataset.afterLogin) || '/members',
    // Where the auth pages live in Webflow. Override per site with
    // data-signup-path / data-login-path on the script tag. Defaults match
    // blbd-2's real slugs; a site using Webflow's native User Accounts
    // feature (slug /log-in, with a hyphen) needs to override this one.
    signupPath: (script && script.dataset.signupPath) || '/sign-up',
    loginPath: (script && script.dataset.loginPath) || '/login',
    // Where a signed-in member's own "account-link" nav item goes.
    // Override with data-members-path / data-members-link-text.
    membersPath: (script && script.dataset.membersPath) || '/members',
    membersLinkText: (script && script.dataset.membersLinkText) || 'My Account',
    // Pages only signed-in members may see — a guest who lands on one is
    // redirected to the login page (with ?next=). Override the comma list
    // with data-member-paths. (Member DATA is already protected by Postgres
    // RLS regardless; this is the UX gate on top of that.)
    memberPaths: ((script && script.dataset.memberPaths) || '/members,/goals,/profile,/community')
      .split(',')
      .map(function (s) {
  ...
  function applyState() {
    ...
    // Re-stamp first, not last: the pre-paint head snippet already guessed
    // this from localStorage, and the CSS keyed on it is hiding/showing
    // things right now. Correcting it before the DOM work below means a
    // wrong guess (revoked token, expired refresh) is undone in the same
    // frame rather than a beat later. See webflow/head-snippet.html.
    ...
    // One nav link, one attribute, no duplication needed: a "Join BLBD" (or
    // similar) link tagged data-blbd="account-link" points at signup while
    // signed out, then automatically repoints to the members page and
    // relabels itself once signed in. This is the low-friction alternative
    // to the anon-only/member-only two-link pattern in DESIGN-TEAM.md — use
    // whichever is easier to set up on a given site.
    document.querySelectorAll('[data-blbd="account-link"]').forEach(function (el) {
      el.setAttribute('href', loggedIn ? CFG.membersPath : CFG.signupPath);
      if (loggedIn) el.textContent = CFG.membersLinkText;
    });

    mountNavAccount(loggedIn);
  }

  // ---------------------------------------------------------------
  // Nav account menu — swaps the "Join BLBD" nav button for a profile
  // avatar + dropdown once signed in, with zero Designer work. This is a
  // per-visitor, runtime decision (does *this* browser have a session?), so
  // it can only live here, not in the static Webflow markup.
  // ---------------------------------------------------------------
  function isNavCta(el) {
    return (
      !!el.closest('nav, [role="banner"], [class*="nav"], [class*="navbar"]') ||
      /\bw-button\b|\bbutton\b|\bbtn\b/i.test(el.className || '')
    );
  }

  // "Join" / "Sign up" style CTAs, reusing the same intent matcher used for
  // auto-routing bare href="#" buttons. Skips anything already tagged with a
  // data-blbd attribute (those are driven by explicit rules elsewhere).
  function findJoinCtas() {
    var found = [].slice.call(document.querySelectorAll('a, button')).filter(function (el) {
      return (
        !el.getAttribute('data-blbd') &&
        authIntent(el.textContent || '') === 'signup' &&
        isNavCta(el)
      );
    });

    // Explicit opt-in wins over text/nav heuristics: `data-blbd-join` marks a
    // signup CTA that this would otherwise miss (different wording, outside
    // the nav). It's also the selector the pre-paint head snippet keys on —
    // see webflow/head-snippet.html.
    [].slice.call(document.querySelectorAll('[data-blbd-join]')).forEach(function (el) {
  ...
  function mountNavAccount(loggedIn) {
    var existing = document.getElementById('blbd-nav-account');

    if (!loggedIn) {
      if (existing) existing.remove();
      // Restore any Join buttons hidden on a previous logged-in render.
      document.querySelectorAll('[data-blbd-join-hidden]').forEach(function (el) {
        el.style.removeProperty('display');
        el.removeAttribute('data-blbd-join-hidden');
      });
      return;
    }
    ...
    var joinEls = findJoinCtas();
    joinEls.forEach(function (el) {
      if (!el.hasAttribute('data-blbd-join-hidden')) {
        el.setAttribute('data-blbd-join-hidden', '1');
        el.style.setProperty('display', 'none', 'important');
      }
    });

    if (existing) {
      updateNavAccountAvatar(existing);
      return;
    }

    if (!joinEls.length) return; // nothing in the nav to stand in for
    var anchor = joinEls[0];
    anchor.insertAdjacentElement('afterend', buildNavAccount());
  }

  function buildNavAccount() {
    var wrap = document.createElement('div');
    wrap.id = 'blbd-nav-account';
    wrap.className = 'blbd-nav-account';

    var btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'blbd-nav-account__btn';
    btn.setAttribute('aria-haspopup', 'true');
    btn.setAttribute('aria-expanded', 'false');
    btn.setAttribute('aria-label', 'Account menu');
    btn.innerHTML =
      '<span class="blbd-nav-account__avatar"></span><span class="blbd-nav-account__caret" aria-hidden="true">▾</span>';

    var menu = document.createElement('div');
    menu.className = 'blbd-nav-account__menu';
    menu.setAttribute('role', 'menu');
    menu.innerHTML =
      '<div class="blbd-nav-account__head">' +
      '<div class="blbd-nav-account__name"></div>' +
      '<div class="blbd-nav-account__email"></div></div>' +
      '<a role="menuitem" href="' + esc(CFG.membersPath) + '">Dashboard</a>' +
      '<a role="menuitem" href="/goals">Goals</a>' +
      '<a role="menuitem" href="/profile">Profile</a>' +
      '<a role="menuitem" href="/community">Community</a>' +
      '<button role="menuitem" type="button" data-blbd="logout">Log out</button>';
    ...
    updateNavAccountAvatar(wrap);
    return wrap;
  }

  function updateNavAccountAvatar(wrap) {
    var email = (session && session.user && session.user.email) || '';
    var name = (profile && profile.display_name) || email.split('@')[0] || 'Member';
    var av = wrap.querySelector('.blbd-nav-account__avatar');

    if (profile && profile.avatar_url) {
      av.innerHTML = '<img src="' + esc(profile.avatar_url) + '" alt="">';
    } else {
      av.textContent = name.charAt(0).toUpperCase();
    }

    var nameEl = wrap.querySelector('.blbd-nav-account__name');
    if (nameEl) nameEl.textContent = name;
    var emailEl = wrap.querySelector('.blbd-nav-account__email');
    if (emailEl) emailEl.textContent = email;
  }

  // Bounce guests off member-only pages, before the page meaningfully
  // renders. Returns true if it redirected (so boot can skip start()).
  function currentMemberPath() {
    return location.pathname.replace(/\/+$/, '') || '/';
  }

  function isMemberPath() {
    return CFG.memberPaths.indexOf(currentMemberPath()) !== -1;
  }

  function gateMemberPages() {
    if (BLBD.isLoggedIn()) return false;
    if (!isMemberPath()) return false;
    window.location.replace(
      CFG.loginPath + '?next=' + encodeURIComponent(location.pathname + location.search),
    );
    return true;
  }

  // ---------------------------------------------------------------
  // Member portal sidebar — a right-docked, collapsible vertical menu shown
  // only to signed-in members on member pages. Complements the top-nav
  // avatar dropdown with always-visible portal navigation. SDK chrome, so
  // it lives here rather than being pasted into each Webflow page.
  // ---------------------------------------------------------------
  var SIDEBAR_ITEMS = [
    { href: CFG.membersPath, icon: '◈', label: 'Dashboard' },
    { href: '/goals', icon: '✹', label: 'Goals' },
    { href: '/profile', icon: '◉', label: 'Profile' },
    { href: '/community', icon: '❋', label: 'Community' },
  ];

  function mountMemberSidebar() {
    if (!BLBD.isLoggedIn() || !isMemberPath()) return;
    if (document.getElementById('blbd-sidebar')) return;
    injectStyles();

    var collapsed = false;
    try {
      collapsed = localStorage.getItem('blbd.sidebar.collapsed') === '1';
    } catch (e) {}

    var here = currentMemberPath();
    var bar = document.createElement('aside');
    bar.id = 'blbd-sidebar';
    bar.className = 'blbd-sidebar';
    bar.setAttribute('data-collapsed', collapsed ? 'true' : 'false');
    bar.setAttribute('aria-label', 'Member portal');

    var toggle = document.createElement('button');
    toggle.type = 'button';
    toggle.className = 'blbd-sidebar__toggle';
    toggle.setAttribute('aria-label', 'Toggle portal menu');

    var nav = document.createElement('nav');
    nav.className = 'blbd-sidebar__nav';
    nav.innerHTML =
      SIDEBAR_ITEMS.map(function (item) {
        var active = item.href.replace(/\/+$/, '') === here;
        return (
          '<a href="' + esc(item.href) + '"' + (active ? ' aria-current="page"' : '') + '>' +
          '<span class="blbd-sidebar__icon" aria-hidden="true">' + item.icon + '</span>' +
          '<span class="blbd-sidebar__label">' + esc(item.label) + '</span></a>'
        );
      }).join('') +
      '<button type="button" data-blbd="logout">' +
      '<span class="blbd-sidebar__icon" aria-hidden="true">⎋</span>' +
      '<span class="blbd-sidebar__label">Log out</span></button>';
    ...
    toggle.addEventListener('click', function () {
      var next = bar.getAttribute('data-collapsed') !== 'true';
      bar.setAttribute('data-collapsed', next ? 'true' : 'false');
      try {
        localStorage.setItem('blbd.sidebar.collapsed', next ? '1' : '0');
      } catch (e) {}
    });
  }
    ...
    function composerHtml() {
      if (!BLBD.isLoggedIn()) {
        return (
          ...
          '<a class="blbd-c-btn" href="' + esc(CFG.loginPath) + '?next=' +
          encodeURIComponent(location.pathname) +
          '">Log in</a> ' +
          '<a class="blbd-c-btn blbd-c-btn--ghost" href="' + esc(CFG.signupPath) + '">Create account</a></div>'
        );
      }
      if (!BLBD.hasPaidTier()) {
        return (
  ...
  function injectStyles() {
    ...
    css.textContent = [
      ...
      // --- nav account menu (Join button replacement when signed in) ---
      '.blbd-nav-account{position:relative;display:inline-flex;align-items:center}',
      '.blbd-nav-account__btn{display:inline-flex;align-items:center;gap:5px;background:none;border:none;cursor:pointer;padding:2px 4px 2px 2px;border-radius:999px;font-family:inherit}',
      '.blbd-nav-account__caret{font-size:11px;color:#69778c}',
      '.blbd-nav-account__avatar{width:38px;height:38px;border-radius:50%;overflow:hidden;background:#48468c;color:#fff;display:grid;place-items:center;font-weight:700;font-size:15px;font-family:inherit;flex:none}',
      '.blbd-nav-account__avatar img{width:100%;height:100%;object-fit:cover;display:block}',
      '.blbd-nav-account__menu{position:absolute;right:0;top:calc(100% + 8px);min-width:224px;background:#fff;border:1px solid #e4e8ed;border-radius:12px;box-shadow:0 12px 40px rgba(40,38,77,.18);padding:6px;z-index:9999;display:none;font-family:inherit}',
      '.blbd-nav-account.is-open .blbd-nav-account__menu{display:block}',
      '.blbd-nav-account__menu a,.blbd-nav-account__menu button{display:block;width:100%;text-align:left;padding:9px 12px;margin:0;border:none;background:none;border-radius:8px;font:inherit;font-size:14px;color:#28264d;text-decoration:none;cursor:pointer}',
      '.blbd-nav-account__menu a:hover,.blbd-nav-account__menu button:hover{background:#f7f8fa}',
      '.blbd-nav-account__head{padding:8px 12px 10px;border-bottom:1px solid #e4e8ed;margin-bottom:6px}',
      '.blbd-nav-account__name{font-weight:700;font-size:14px;color:#28264d}',
      '.blbd-nav-account__email{font-size:12px;color:#69778c;word-break:break-all}',
      // --- member portal sidebar (right-docked, collapsible) ---
      '.blbd-sidebar{position:fixed;top:50%;right:0;transform:translateY(-50%);z-index:9998;display:flex;align-items:flex-start;font-family:inherit}',
      '.blbd-sidebar__toggle{align-self:center;width:22px;height:52px;border:1px solid #e4e8ed;border-right:none;border-radius:10px 0 0 10px;background:#fff;color:#69778c;cursor:pointer;font-size:12px;padding:0;box-shadow:-2px 0 10px rgba(40,38,77,.06)}',
      '.blbd-sidebar__toggle::before{content:"›"}',
      '.blbd-sidebar[data-collapsed="true"] .blbd-sidebar__toggle::before{content:"‹"}',
      '.blbd-sidebar__nav{display:flex;flex-direction:column;gap:2px;padding:8px;background:#fff;border:1px solid #e4e8ed;border-right:none;border-radius:12px 0 0 12px;box-shadow:-6px 0 24px rgba(40,38,77,.1);width:190px;overflow:hidden;transition:width .18s ease,padding .18s ease,opacity .18s ease}',
      '.blbd-sidebar[data-collapsed="true"] .blbd-sidebar__nav{width:0;padding:8px 0;opacity:0;pointer-events:none}',
      '.blbd-sidebar__nav a,.blbd-sidebar__nav button{display:flex;align-items:center;gap:10px;width:100%;text-align:left;padding:10px 12px;border:none;background:none;border-radius:8px;font:inherit;font-size:14px;color:#28264d;text-decoration:none;cursor:pointer;white-space:nowrap}',
      '.blbd-sidebar__nav a:hover,.blbd-sidebar__nav button:hover{background:#f7f8fa}',
      '.blbd-sidebar__nav a[aria-current="page"]{background:rgba(102,149,242,.12);color:#48468c;font-weight:600}',
      '.blbd-sidebar__icon{width:18px;text-align:center;flex:none;color:#6695f2}',
      '@media(max-width:767px){.blbd-sidebar{top:auto;bottom:16px;transform:none}}',
  ...
  function gate(message, cta) {
    return (
      ...
      esc(message) +
      '</p>' +
      (cta || '<a class="blbd-c-btn" href="' + esc(CFG.loginPath) + '?next=' + encodeURIComponent(location.pathname) + '">Log in</a>') +
      '</div>'
    );
  }
  ...
  ready
    ...
    .catch(function () {})
    .then(function () {
      // Gate member-only pages before doing anything else — a guest on
      // /members etc. is redirected to login and never renders the page.
      if (gateMemberPages()) return;
